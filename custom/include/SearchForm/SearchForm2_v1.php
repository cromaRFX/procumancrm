  ...
  function displaySavedSearchSelect(){
    ...
    protected function parseDateExpression($operator, $db_field, $field_type = '')
    {
     ...
     public function generateSearchWhere($add_custom_fields = false, $module='') {
		...
		if($this->module == 'AOS_Products')
		{
			$GLOBALS['log']->fatal("Products Hard Coded Filter...");
			//Hard Coded Filter for Expired Products
			global $timedate;
			$today = $timedate->getInstance()->nowDbDate();
			$expiry_field = 'aos_products_cstm.trade_license_expiry_date_c'; 
			$where .=  " (".$expiry_field." > '".$today."') OR (".$expiry_field." IS NULL OR ".$expiry_field." = '0000-00-00')";
		...
		$GLOBALS['log']->fatal("Before Return Where ...".print_r($where_clauses,1));
