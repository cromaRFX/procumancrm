...
class CustomAOS_QuotesViewDetail extends ViewDetail {
     ...
     */
    function CustomAOS_QuotesViewDetail(){
	...
	function display(){
		// Good Receipt Rights
		$this->ss->assign("SHOW_GOODRECEIVED_BTN", false);
		
		if(ACLController::checkAccess('gdrcp_Goods_Receipt', 'edit', true)) {
			$this->ss->assign("SHOW_GOODRECEIVED_BTN", true);
		}
		$this->populateQuoteTemplates();
		$this->displayPopupHtml();
		
		/* 
			start: Adding Recieve invoice button : waleed
		*/
		global $mod_strings;
		$receiveInvoiceBtn = '<input title="' .$mod_strings['LBL_RECIEVE_INVOICE']. '" class="button" onclick="checkPOstatus();" type="button" name="trigger" value="'.$mod_strings['LBL_RECIEVE_INVOICE'].'" >';
		$this->dv->ss->assign("RECEIVE_INVOICE_BTN", $receiveInvoiceBtn);
		/* 
			end: Adding Recieve invoice button 
		*/
