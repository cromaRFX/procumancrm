...
function display_lines($focus, $field, $value, $view){
    ...
    if($view == 'EditView'){
        ...
        $html .= '<input type="hidden" name="vathidden" id="vathidden" value="'.get_select_options_with_id($app_list_strings['vat_list'], '').'">
                  ...
                  <input type="hidden" name="payterms" id="payterms" value="'.get_select_options_with_id($app_list_strings['payment_terms_c'], '').'">
                  <input type="hidden" name="uom_c" id="uom_c" value="'.get_select_options_with_id($app_list_strings['UOM_Options'], '').'">';
        if($focus->id != '') {
        ...
        while ($row = $focus->db->fetchByAssoc($result)) {
            ...
            if($enable_groups && ($group_id != $row['group_id'] || $i == 0)){
                ...
                if($i != 0)$html .= "<tr><td colspan='10' nowrap='nowrap'><br></td></tr>";
                $groupStart = '';
                $groupEnd = '';
                $product = '';
                $service = '';
            ...
            if($line_item->product_id != '0' && $line_item->product_id != null){
                if($productCount == 0)
                {
                    ...
                    $product .= "<td width='6%' class='tabDetailViewDL' style='text-align: left;padding:2px;' scope='row'>".$mod_strings['LBL_PRODUCT_QUANITY']."</td>";
                    $product .= "<td width='12%' class='tabDetailViewDL' style='text-align: left;padding:2px;' scope='row'>".$mod_strings['LBL_PRODUCT_NAME']."</td>";
                    $product .= "<td width='8%' class='tabDetailViewDL' style='text-align: left;padding:2px;' scope='row'>".$mod_strings['LBL_COLOR_C']."-".$mod_strings['LBL_SIZE_C']."</td>";
                    ...
                    $product .= "<td width='5%' class='tabDetailViewDL' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_VAT']."</td>";
                ...
                $product .= "<td class='tabDetailViewDF' style='padding:2px;'>".$line_item->color_c."-".$line_item->size_c."</td>";
                ...
                if($serviceCount == 0)
                {
                    ...
                    $service .= "<td width='12%' class='dataLabel' style='text-align: left;padding:2px;'  scope='row'>".$mod_strings['LBL_SERVICE_QTY_C']."</td>";
                ...
                $service .= "<td class='tabDetailViewDF' style='padding:2px;' >".$line_item->product_qty."</td>";
