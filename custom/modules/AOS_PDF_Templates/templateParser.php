    ...
    function parse_template_bean($string, $key, &$focus)
    {
        ...
        foreach ($focus->field_defs as $field_def) {
            if (isset($field_def['name']) && $field_def['name'] != '') {
                ...
                if ($field_def['type'] == 'currency') {
				...
				else if ($field_def['name'] == 'line_items_c') {
					$multi_lines 	= explode('^|^',$focus->line_items_c);
					$lineItems='<table border="1" style="width:100%; border-collapse: collapse; border: 1px solid #000000;">
					<tr>
					...
					<td style="width:29%;"><strong>Description</strong></td>
					</tr>';
					foreach($multi_lines as $index=>$line_items)
					{
						$line_item = explode('^,^',$line_items);
						$lineItems.='<tr> 
							<td style="width:5%;">'.$line_item['0'].'</td>
							<td style="width:13%;">'.$line_item['7'].'</td>
							<td style="width:13%;">'.$line_item['5'].'</td>
							<td style="width:10%;">'.$line_item['2'].'</td>
							<td style="width:10%;">'.$line_item['3'].'</td>
							<td style="width:10%;">'.$line_item['4'].'</td>
							<td style="width:10%;">'.$line_item['9'].'</td>
							<td style="width:29%;">'.$line_item['1'].'</td>
							</tr>';
					}
					$lineItems.='</table>';
					// $GLOBALS['log']->fatal("lineItems: ".$lineItems);
					$repl_arr[$key . "_" . $fieldName]=$lineItems;
				}
				else {
