...
function generateComposeDataPackage($data, $forFullCompose = TRUE)
{
    ...
    if (isset($data['parent_type']) && !empty($data['parent_type']) &&
		...
		if (isset($data['cc_addrs']) || isset($data['bcc_addrs'])) {
			$namePlusEmailCC = $data['cc_addrs'];
			$namePlusEmailCC = from_html(str_replace("&nbsp;", " ", $namePlusEmailCC));
			$namePlusEmailBCC = $data['bcc_addrs'];
			$namePlusEmailBCC = from_html(str_replace("&nbsp;", " ", $namePlusEmailBCC));
		}
