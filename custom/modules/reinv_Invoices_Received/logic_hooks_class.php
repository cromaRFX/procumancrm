    ...
    class logic_hooks_class
    {
        ...
        {		
		   ...
		   if ($curr_id=='-99') {
			...
			$bean->discount_amount_default_c=0;
			$bean->shipping_amount_default_c=0;
			$bean->subtotal_amount_default_c=0;
			$bean->total_amount_default_c=0;
			$bean->tax_amount_default_c=0;
			}
			...
			if( !empty($_REQUEST['aos_quotes_id_c']) && $_REQUEST['via_quotes']=='yes'){
				...
				foreach ( $linteItems as $linteItem ) {
					...
					foreach ( $goodRecipts as $goodRecipt ) { 
						...
						$db->query("UPDATE gdrcp_goods_receipt_cstm set quantity_invoiced_c='".$goodRecipt->quantity_ordered_c."' , invoiced_amount_c='".$linteItem->product_total_price."' WHERE id_c='".$goodRecipt->id."' ");
