...
<script type="text/javascript" language="javascript">
	...
	function addMultiLineRow()
	{
	  ...
	  */
		YAHOO.util.Event.addListener('porq_Purchase_Request__line_item_price__'+cur_count, 'change', function(){
				var lineRow = this.id.split("porq_Purchase_Request__line_item_price__");
				rowNum=lineRow['1'];
				...
				if( !isNaN(parseFloat(result)) )
					...
					$(this).val('0.00');
				var result2=parseFloat(this.value*$('#porq_Purchase_Request__line_item_qty__'+rowNum).val() ).toFixed(2);
				if( !isNaN(parseFloat(result)) )
					$('#porq_Purchase_Request__line_item_cost__'+rowNum).val(parseFloat(result2).toFixed(2));
				else
					$('#porq_Purchase_Request__line_item_cost__'+rowNum).val('0.00');
				calculateRequestedTotal();
		});
		// on product change poulate currency
		YAHOO.util.Event.addListener('porq_Purchase_Request__line_item_product_id__'+cur_count, 'change', function(){
			var lineRow = this.id.split("porq_Purchase_Request__line_item_product_id__");
			rowNum=lineRow['1']; 
	...
	$( document ).ready(function() {
		making2decimalFields();
		//cost on price change from any where
		$('input[id^="porq_Purchase_Request__line_item_price__"]').each(function () {
		...
		// on product change poulate currency
		$('input[id^="porq_Purchase_Request__line_item_product_id__"]').each(function () {
			YAHOO.util.Event.addListener(this, 'change', function(){
				var lineRow = this.id.split("porq_Purchase_Request__line_item_product_id__");
				rowNum=lineRow['1']; 
				getProductCurrency(this.value,rowNum);
	...
	*/
	function making2decimalFields(){
		 $('input[id^="porq_Purchase_Request__line_item_price__"],input[id^="porq_Purchase_Request__line_item_cost__"]').each(function () {
	...
	*/
	function calculateRequestedTotal(){
		...
		$('input[id^="porq_Purchase_Request__line_item_currency__"]:visible').each(function () {
	...
	*/
	...
	function getProductCurrencySuccess(data){
		...
		$('#porq_Purchase_Request__line_item_currency__'+ddArray['0']['rowN']).val(ddArray['0']['currencyName']);
		
		// if any of line item currency is not default currency set trquested total sum to 0.00
		if(ddArray['0']['currencyName']!=defaultCurrency){
			$('#total_cost').val('0.00');
		}	
...
SUGAR.util.doWhen(
      ...
      <tr>
        <td valign="top" id="{$module}__line_item_description__{$multi_lines_count}_label" scope="col"> Service/Goods Description: </td>
         <td valign="top" width="37.5%"><input type="text" name="{$module}__line_item_description[{$multi_lines_count}]" id="{$module}__line_item_description__{$multi_lines_count}" size="30" maxlength="255" value="{$item.1}" title=""></td>
