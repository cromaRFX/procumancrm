...
function generateFile($fileConfig, $reportName, $module, $description, $displayTitles, $displayHeaders, $headers, $resultset, $resultsetNoFormat, $headersTotals, $totals, $subTotals, $isDetailedReport, $isGroupedReport, $hasGroupedTotalBelowColumn, $pdf_orientation, $pngs, $legendChart, $engineChart, $isHTML, $pdf_img_scaling_factor, $reportDate, $userTZ, $rowIndex, $report_charts, $columnsDataTypes, $columnsDataFunctions, $columnsDataVisible, $columnsDataWidths, $currentReportCss, $domainId,$pdf_pageFormat) {
	...
	if (count($pngs) > 0) {
		...
		$generatedChartImages = generateChartImages($isHTML, $pngs, $legendChart, $engineChart, $htmlCss,$encoding);
	...
	if (!$isDetailedReport) {
		...
		if ($displayHeaders) {
			...
			foreach ($headers as $column) {
				...
				$column = ($isHTML ? $column : str_replace('<', '&lt;', $column));
		...
		foreach ($resultset as $key0=>$row){
			...
			foreach ($row as $key=>$value){
				...
				if (!$isHTML && in_array($dataType, array('bool', 'headerButton', 'noHeaderButton'))) {
					$value = $resultsetNoFormat[$key0][$key];
				}
			...
			foreach ($headers as $column) {
				...
				foreach ($totals as $total) {
					...
					if (isset($total[$column])) {
						...
						$total[$column] = ($isHTML ? $total[$column] : str_replace('<', '&lt;', $total[$column]));
		...
		foreach ($resultset as $key=>$subGroup) {
			...
			foreach ($subGroup as $key0=>$row) {
				...
				foreach ($row as $key2=>$value){
					...
					if (!$isHTML && in_array($dataType, array('bool', 'headerButton', 'noHeaderButton'))) {
						$value = $resultsetNoFormat[$key0][$key2];
					}
				...
				if ($isGroupedReport && $hasGroupedTotalBelowColumn) {
					...
					foreach ($headers as $column) {
						...
						if (isset($subTotals[$key][$column])) {
							...
							$subTotals[$key][$column] = ($isHTML ? $subTotals[$key][$column] : str_replace('<', '&lt;', $subTotals[$key][$column]));
	...
	if ($isHTML) {
		...
		fwrite($descriptor, asol_ReportsUtils::unicodeDecode($html));
		fclose($descriptor);
		...
		if ( !$pdfExportNativeLibrary && is_file($sugar_config["asolReportsPhantomJsFilePath"])) {
			...
			shell_exec($sugar_config["asolReportsPhantomJsFilePath"].' modules/asol_Reports/include/js/phantomjs/asolRasterize.js '.$tmpFilesDir.'Pdf_Trest.html '.$currentDir.$tmpFilesDir.$idFile.' '.$pdf_pageFormat.' '.$pdf_orientation);
		}else{
			...
			$pdf->writeHTML(asol_ReportsUtils::unicodeDecode($htmlCss.$html), true, false, true, false);
...
function generateChartImages($isHTML, $pngs, $legendChart, $engineChart, $htmlCss,$encoding) {
	...
	foreach ($pngs as $key => $png) {
		...
		$png = asol_ReportsUtils::unicodeDecode($png);
