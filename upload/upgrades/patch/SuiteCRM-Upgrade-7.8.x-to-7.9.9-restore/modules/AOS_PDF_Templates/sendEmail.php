...
class sendEmail{
	...
	global $current_user, $mod_strings, $sugar_config;
		...
		//RFx Module, Get Accounts Primary address and populate in To Field
		if($module_type == 'RFX_RFx')
		{
			$accounts = $module->get_linked_beans('rfx_rfx_accounts_1','Accounts');
			if(count($accounts) > 0)
			{
				foreach ($accounts as $key => $account) 
				{
					$account_email = $account->emailAddress->getPrimaryAddress($account);
					$emailBcc[] = $account_email;
				}
				//Since To Address is shown, we shall populate it
				$email->to_addrs	= implode(',',$emailBcc);
				//$email->bcc_addrs 	= implode(',',$emailBcc);
			}
		}
