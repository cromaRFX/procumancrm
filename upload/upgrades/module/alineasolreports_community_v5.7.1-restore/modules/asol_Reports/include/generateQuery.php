...
class asol_ReportsGenerateQuery {
	...
	static public function getSqlJoinQuery(& $fieldValues, & $filterValues, $hasDeletedUsage, $resultsLimit, $reportModule, $reportTable, $auditedReport, $alternativeDb, & $fieldsByRef, $aliasSuffix = '', & $domainField = null) {
		...
		if (!isset($_SESSION['asolReportsTableIndexes'][$dbKey][$reportTable])) {
			foreach (asol_Reports::getSelectionResults("SHOW INDEX FROM ".$reportTable, null, false, $alternativeDb) as $tableCurrentIndex) {
		...
		foreach ($fieldValues['tables'][0]['data'] as $index => & $currentValues) {
			...
			//**************************************//
			$fieldIsVisible = (in_array(strtolower($currentValues['visible']), array("yes", "html", "internal")) && self::propertyApplies($currentValues['visibilityConfig']) && self::roleApplies($currentValues['roles']) ); 
			...
			foreach($keyExploded as $keyDepth=>$currentLink) {
				...
				if ($isNonCrmDatabase) {
					...
					if ($currentLinkKey === 'id') { //Relationship
						...
						$mainTableModule = ($keyDepth >= 1 ? $tableModulesAssociation[$mainTableName] : $reportModule);
						...
						$availableRelationShips = self::getAvailableRelationShips($mainTableModule, $mainTableName, $relatedTableName, $currentLinkKey);
						...
						if ($isVirtualLink) {
							...
							$relatedInfo = asol_Reports::getReportsRelatedFields($bean, $currentLink);
				...
				if (!isset($_SESSION['asolReportsTableIndexes'][$dbKey][$relatedTable])) {
					foreach (asol_Reports::getSelectionResults("SHOW INDEX FROM ".$relatedTable, null, false, $alternativeDb) as $tableCurrentIndex) {
	...
	static private function getAvailableRelationShips($mainModule, $mainTable, $relatedTable, $fieldKey) {

		global $sugar_config;
		
		if (!isset($sugar_config['asolReportsRelationshipsTableUsage']) || !$sugar_config['asolReportsRelationshipsTableUsage']) {
		
			$relationshipsR2L = asol_CommonUtils::getRelationshipsInfoList($mainModule, null, null, $relatedTable, $mainTable, null, $fieldKey);
			$relationshipsL2R = asol_CommonUtils::getRelationshipsInfoList($mainModule, null, null, $mainTable, $relatedTable, $fieldKey, null);
		
			$availableRelationShips = array_merge($relationshipsR2L, $relationshipsL2R);
			...
			$relationships_R2L = "SELECT DISTINCT relationship_name, lhs_table, lhs_key, rhs_table, rhs_key, join_table, join_key_lhs, join_key_rhs, relationship_role_column, relationship_role_column_value FROM relationships WHERE rhs_table='".$mainTable."' AND lhs_table='".$relatedTable."' AND rhs_key='".$fieldKey."'";
			...
			$relationships_L2R = "SELECT DISTINCT relationship_name, lhs_table, lhs_key, rhs_table, rhs_key, join_table, join_key_lhs, join_key_rhs, relationship_role_column, relationship_role_column_value FROM relationships WHERE lhs_table='".$mainTable."' AND rhs_table='".$relatedTable."' AND lhs_key='".$fieldKey."'";
	...
	static private function fieldSqlVarsInFilters($fieldValues, $filterValues, $fieldInfo) {
		...
		$beanItems = asol_CommonUtils::getItemVariables($fieldInfo['sql']);
	...
	static public function getSqlSelectQuery(& $fieldValues, & $chartInfo, $predefinedTemplates, $alternativeDb, $reportTable, $hourOffset, $quarterMonth, $weekStart, $audited_report, $displayTotals, $displaySubtotals, $fieldsByRef, $filtersByRef, $aliasIndexTable = array(), $aliasSuffix = '') {
		...
		foreach ($fieldValues['tables'][0]['data'] as $i => & $currentValues) {
			...
			//***AlineaSol Premium***//
			//***********************//
			$extraParams = array('label' => $currentValues);
			...
			$currentValues['enumLabels'] = (in_array($currentValues['enumOperator'], array('options', 'function'))) ? asol_Reports::getEnumLabels($currentValues['enumOperator'], $currentValues['enumReference']) : array();	
			
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
			$extraParams = array('extraFormat' => $currentValues['format']['extra'], 'template' => $currentValues['templates']['enum'], 'enumTemplates' => $predefinedTemplates['enum']);
			$enumLabels = asol_CommonUtils::managePremiumFeature("commonEnumFormat", "commonFunctions.php", "getCommonEnumValues", $extraParams);
			...
			//***AlineaSol Premium***//
			//***********************//
			$extraParams = array('currentTree' => $predefinedTemplates['tree'][$currentValues['format']['extra']['tree']]['content']['data']);
			$returnedPremiumTree = asol_CommonUtils::managePremiumFeature("commonTreeFormat", "commonFunctions.php", "getCommonTreeFormattedValues", $extraParams);
			$currentValues['treeValues'] = ($returnedPremiumTree !== false ? $returnedPremiumTree : null);
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
			...
			if (in_array($currentValues['function'], array("0", "undefined"))) {
				...
				if (!empty($sqlFunction)) { //ASOL CALCULATED
					...
					if (in_array(strtolower($currentValues['visible']), array("yes", "html", "internal")) && self::propertyApplies($currentValues['visibilityConfig']) && self::roleApplies($currentValues['roles']))
			...
			foreach ($chartInfo as $cKey=>$cInfoData) {
				...
				$cInfoData['label'] = (is_array($cInfoData['label']) ? $cInfoData['label']['text'] : $cInfoData['label']);
	...
	static public function generateSqlWhere(& $fieldValues, $filterValues, $reportTable, $hourOffset, $quarterMonth, $weekStart, $fieldsByRef, & $filtersByRef, $avoidTrim, $aliasSuffix = '') {
		...
		if (!empty($filterValues)) {
			...
			foreach($filterValues as & $filter) {
				...
				$appliesFilter = self::modifyFilteringValues($filter, $quarterMonth, $weekStart, $reportTable, $hourOffset, $operator1, $operator2, $avoidTrim, $aliasSuffix);			
				self::populateFilteringValues($filter, $operator1, $operator2, $appliesFilter, $filtersByRef);
	...
	static private function modifyFilteringValues(& $filter, $quarterMonth, $weekStart, $reportTable, $hourOffset, & $operator1, & $operator2, $avoidTrim, $aliasSuffix) {

		global $current_user, $timedate;
		
		$applyFilter = true;
		
		//Add report table & ".", if fields belong to main table
		...
		$hasTime = (in_array($filter['type'], array("datetime", "datetimecombo", "timestamp")) ? "+1 days": "");
		
		if (!$avoidTrim) {
			$filter['parameters']['first'][0] = trim($filter['parameters']['first'][0]);
		}
		...
		switch ($filter['operator']) {
				...
				if (in_array($filter['type'], array("date", "datetime", "datetimecombo", "timestamp"))) {
					...
					switch ($filter['parameters']['first'][0]) {
						...
						case "calendar":
							...
							if ($hasOffset && strtotime($filter['parameters']['first'][0])!='') {
			...
			case "not like":
				...
				$operator2 = "";
				$filter['parameters']['first'][0] = preg_replace('/_/', "\\_", $filter['parameters']['first'][0]);
				$filter['parameters']['first'][0] = strpos($filter['parameters']['first'][0], "%") !== false ? $filter['parameters']['first'][0] : "%".$filter['parameters']['first'][0]."%";
				
				$applyFilter = ($filter['operator'] !== "like") || ($filter['parameters']['first'][0] !== "%%");
				break;
			...
			case "not between":
				...
				switch ($filter['parameters']['first'][0]) {
					...
					case "calendar":
						...
						if ($hasOffset) {
							$filter['parameters']['first'][0] = (!empty($filter['parameters']['first'][0] ) && strtotime($filter['parameters']['first'][0])!='' ? date("Y-m-d H:i:s", strtotime($hourOffset." seconds", strtotime($filter['parameters']['first'][0]))) : $filter['parameters']['first'][0]);
							$filter['parameters']['second'][0] = (!empty($filter['parameters']['second'][0] ) && strtotime($filter['parameters']['second'][0])!='' ? date("Y-m-d H:i:s", strtotime(((24*3600+$hourOffset)-1)." seconds", strtotime($filter['parameters']['second'][0]))) : $filter['parameters']['second'][0]);
						} else {
							$filter['parameters']['second'][0] = (!empty($filter['parameters']['second'][0]) && strtotime($filter['parameters']['second'][0])!='' ? date("Y-m-d", strtotime($filter['parameters']['second'][0].$hasTime) ) : $filter['parameters']['second'][0] );
						}
						
						$applyFilter = (!empty($filter['parameters']['first'][0]) && !empty($filter['parameters']['second']));
			...
			case "last":
				switch ($filter['parameters']['first'][0]) {
					...
					case "week":
						...
						$currentHour = ((date("G")*3600)+$hourOffset)/3600;
						$diaSemana = 0;
						if ($currentHour < 0){
							$diaSemana = date("N", time()+$hourOffset);
						} else {
			...
			//In case that there's an old report, keep fixed_date "deprecated" options
			case "fixed date":
	...
	static private function populateFilteringValues($filter, $operator1, $operator2, $appliesFilter, & $filtersByRef) {
		...
		$hasParenthesis = (!empty($filterLogicalOperators['parenthesis']));
		if ($appliesFilter || $hasParenthesis) {
			...
			if (((in_array($filter['type'], array("enum", "radioenum"))) && (in_array($filter['operator'], array("one of", "not one of")))) || (in_array($operator1, array("IN", "NOT IN")))) {
				...
				if (in_array($filter['type'], $numericTypes)) {
					if ($filter['parameters']['first'][0] != "") {
						if (in_array($operator1, array("LIKE", "NOT LIKE"))) {
							$currentSqlWhere .= $filterStaticVar." ".$operator1." '".str_replace("'", "\'", $filter['parameters']['first'][0])."' ";
						} else {
		...
		foreach ($fieldValues['tables'][0]['data'] as $i=>$currentValues) {
			...
			if (in_array($currentValues['grouping'], array("Grouped", "Minute Grouped", "Quarter Hour Grouped", "Hour Grouped", "Day Grouped", "DoW Grouped", "WoY Grouped", "Month Grouped", "Natural Quarter Grouped", "Fiscal Quarter Grouped", "Natural Year Grouped", "Fiscal Year Grouped"))) {
				...
				$groups[] = array(
					...
					'reference' => $currentValues['fieldReference'],
	...
	static public function getSqlOrderByQuery($fieldValues, $reportTable, $alternativeDb, $isGroupedReport, $maxAllowedNotIndexedOrderBy, $fieldsByRef, & $sortField = "", & $sortDirection = "", & $sortIndex = "") {
		...
		if ($sortField !== "") {
			...
			foreach ($fieldValues['tables'][0]['data'] as $currentValues) {
				...
				if (($currentValues['originalFieldName'] == $sortField) && ($currentValues['index'] == $sortIndex)) {
					...
					$extraParams = array('label' => $currentValues);
					$returnedPremiumAlias = asol_ReportsUtils::managePremiumFeature("multiLanguageReport", "reportFunctions.php", "getMultiLanguageLabel", $extraParams);
					$sortAlias = ($returnedPremiumAlias !== false ? $returnedPremiumAlias : $currentValues['alias']);
					//***********************//
					//***AlineaSol Premium***//
					//***********************//
					
					if ($currentValues['format']['type'] == 'button' && isset($currentValues['format']['extra']['label'])) {
						preg_match("/^%{(.*?)}/", $currentValues['format']['extra']['label'], $fieldRef);
						if (count($fieldRef) > 1) { 
							$fieldRef = $fieldRef[1];
					 		$sortableField = $fieldsByRef[$fieldRef]['field'];
						}
					}
					
					$sortableField = (isset($sortableField) ? $sortableField : $currentValues['field']);
					
					$sortType = $currentValues['type'];
					$sqlOrder = " ORDER BY ".$sortableField." ".$sortDirection." ";
					break;
			...
			$sortField = explode('.',$orders[0]['field']);
			$sortField = $sortField[1];
			$sortDirection =  $orders[0]['dir'];
		}
	...
	static public function getSqlSubSetLimitQuery($alternative_database, $results_limit, $totalEntries, $entriesPerPage, $pageNumber, $report_table, $table_primary_key, $sqlFrom, $sqlJoin, $sqlWhere, $sqlGroup, $sqlHaving) {
		...
		if ($results_limit == "all"){
			...
			if ($res_limit[1] == 'first') {
				...
				$limitedTotalsRs = asol_Reports::getSelectionResults($sqlLimitedTotals, null, $useExternalDbConnection, $alternativeDb);
	...
	static public function getExternalTablePrimaryKey($alternative_database, $report_table) {
		...
		$externalColumnsRs = asol_Reports::getSelectionResults("SHOW COLUMNS FROM ".$report_table, null, $useExternalDbConnection, $alternativeDb);
	...
	static public function getSqlLimitDetailWhereQuery($results_limit, $detailGroupFullSize, $report_table, $sqlFrom, $sqlJoin, $sqlDetailWhere, $sqlGroup, $sqlOrder) {
		...
		if ($results_limit == "all") {
			...
			$limitedTotalsRs = asol_Reports::getSelectionResults($sqlLimitedTotals);
	...
	static public function formatGroupTotals($reportId, $dashletId, $rsSubTotals, $totals, $userDateFormat, $userDateTimeFormat, $userTZ, $gmtDates, $auditedReport, $auditedAppliedFields, $auditedFieldType) {
		...
		foreach ($rsSubTotals[0] as $key=>$value) {
			...
			if ($formatType == "date") {
				...
				$calendarType = ( isset($totals[$k]['format']['extra']['calendar']) ? $totals[$k]['format']['extra']['calendar'] : $userConf['calendarType'] );
				...
				if (($value != "") && (date('Y-m-d', strtotime($value)) == $value)) {
					...
					if ($calendarType == 'persian') {
						$rsSubTotals[0][$key] = asol_CommonUtils::dateToPersian($value, false, $userDateFormat);
					} else {
				...
				if (($value != "") && (date('Y-m-d H:i:s', strtotime($value)) == $value)) {
					...
					if ($calendarType == 'persian') {
						$rsSubTotals[0][$key] = asol_CommonUtils::dateToPersian($value, true, $userDateTimeFormat);
					} else {
				...
				$returnedPremiumNumeric = asol_CommonUtils::managePremiumFeature("commonNumericFormat", "commonFunctions.php", "getCommonNumericField", $extraParams);
				$rsSubTotals[0][$key] = ($returnedPremiumNumeric !== false) ? $returnedPremiumNumeric["value"] : $value;
				//***********************//
				//***AlineaSol Premium***//
				//***********************//
				...
				$returnedPremiumNumeric = asol_CommonUtils::managePremiumFeature("commonNumericFormat", "commonFunctions.php", "getCommonNumericField", $extraParams);
				$rsSubTotals[0][$key] = ($returnedPremiumNumeric !== false) ? $returnedPremiumNumeric["value"] : asol_CommonUtils::currency_format($value);
				//***********************//
				//***AlineaSol Premium***//
				//***********************//
				...
				$returnedPremiumNumeric = asol_CommonUtils::managePremiumFeature("commonNumericFormat", "commonFunctions.php", "getCommonNumericField", $extraParams);
				$rsSubTotals[0][$key] = ($returnedPremiumNumeric !== false) ? $returnedPremiumNumeric["value"] : $rsSubTotals[0][$key];
				//***********************//
				//***AlineaSol Premium***//
				//***********************//
				...
				$returnedPremiumPercent = asol_CommonUtils::managePremiumFeature("commonPercentFormat", "commonFunctions.php", "getCommonPercentField", $extraParams);
				...
				$extraParams = array(
					'beanId' => $reportId,
					'sourceModule' => 'asol_Reports',
					'containerClass' => '.asolReportsExecutionRow',
					'currentLabel' => $totals[$k]['alias'],
					'currentValue' => $value,
					'currentRef' => $totals[$k]['fieldReference'],
					'contextId' => $dashletId,
					'extraFormat' => $totals[$k]['format']['extra'],
					'referenceAlias' => null,
					'parentFormat' => false
				);
				$returnedPremiumButton = asol_CommonUtils::managePremiumFeature("commonButtonFormat", "commonFunctions.php", "getCommonButtonField", $extraParams);
				$rsSubTotals[0][$key] = ($returnedPremiumButton !== false ? $returnedPremiumButton['single'] : '');
				//***********************//
				//***AlineaSol Premium***//
				//***********************//
				
			
			} else if ($formatType == "multimedia") {
					...
					$extraParams = array(
							'value' => $value,
							'extra' => $totals[$k]['format']['extra'],
							'currentRef' => $totals[$k]['fieldReference'],
							'currentRow' => $rowValues,
							'referenceAlias' => $referenceAlias,	
					);
					$returnedPremiumImage = asol_CommonUtils::managePremiumFeature("commonMultimediaFormat", "commonFunctions.php", "getCommonMultimediaFormat", $extraParams);
					if ($returnedPremiumImage !== false) {
						$rsSubTotals[0][$key] = $returnedPremiumImage[0];
					}
					//***********************//
					//***AlineaSol Premium***//
					//***********************//
	...
	static public function formatSubGroup($reportId, $dashletId, $subGroup, $subGroupInfo, $userTZ, $gmtDates) {
		...
		if (in_array($formatType, array("enum", "radioenum"))) {
			...
			$returnedPremiumNumeric = asol_CommonUtils::managePremiumFeature("commonNumericFormat", "commonFunctions.php", "getCommonNumericField", $extraParams);
			$subGroup = ($returnedPremiumNumeric !== false) ? $returnedPremiumNumeric["value"] : $subGroup;
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
			...
			$returnedPremiumNumeric = asol_CommonUtils::managePremiumFeature("commonNumericFormat", "commonFunctions.php", "getCommonNumericField", $extraParams);
			$subGroup = ($returnedPremiumNumeric !== false) ? $returnedPremiumNumeric["value"] : asol_CommonUtils::currency_format($subGroup);
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
			...
			$extraParams = array(
				...
				'currentRef' => $subGroupInfo['reference'],
				'contextId' => $dashletId,
				'extraFormat' => $subGroupInfo['format']['extra'],
				'referenceAlias' => null,
				'parentFormat' => false
			...
			$returnedPremiumButton = asol_CommonUtils::managePremiumFeature("commonButtonFormat", "commonFunctions.php", "getCommonButtonField", $extraParams);
			$subGroup = ($returnedPremiumButton !== false ? $returnedPremiumButton['single'] : '');
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
			...
			$extraParams = array(
				'value' => $subGroup,
				'extra' => $subGroupInfo['format']['extra'],
				'currentRef' => $subGroupInfo['reference'],
				'currentRow' => $rowValues,
				'referenceAlias' => $referenceAlias,	
			);
			$returnedPremiumImage = asol_CommonUtils::managePremiumFeature("commonMultimediaFormat", "commonFunctions.php", "getCommonMultimediaFormat", $extraParams);
			if ($returnedPremiumImage !== false) {
				$subGroup = $returnedPremiumImage[0];
			}
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
	...
	static public function formatDateSpecialsGroup($reportId, $dashletId, $subGroup, $fieldInfo, $userDateFormat, $userTZ, $gmtDates) {
		
		global $mod_strings, $timedate, $current_language;
		
		$userConf = asol_CommonUtils::getUserConfiguration();
		$calendarType = $userConf['calendarType'];
		
		switch ($fieldInfo['grouping']) {
								
			case "Detail":

				$subGroup = self::formatSubGroup($reportId, $dashletId, $subGroup, $fieldInfo, $userTZ, $gmtDates);
			...
			case "Quarter Hour Detail":
				...
				if ($calendarType == 'persian' && $current_language == 'fa_ir') {
					asol_CommonUtils::numbersToPersian($subGroup);
				}
			...
			case "Day Detail":
				...
				if ((!$timedate->check_matching_format($subGroup, $userDateFormat)) && ($subGroup != "")){
					...
					if ($calendarType == 'persian') {
						$subGroup = asol_CommonUtils::dateToPersian($currentUnformattedValue, false, $userDateFormat);
					} else {
	...
	static public function formatDetailResultSet($reportId, $dashletId, $subGroups, & $resulsetFields, $userDateFormat, $userDateTimeFormat, $userTZ, $gmtDates, $isGroupedReport, $auditedReport, $auditedAppliedFields, $auditedFieldType, $predefinedTemplates, $referenceAlias, & $availableMassiveButtons) {
		...
		foreach ($subGroups as & $subGroup) {
			...
			foreach ($subGroup as & $values) {
				...
				self::formatResultSetRows($reportId, $dashletId, $resulsetFields, $values, $userDateFormat, $userDateTimeFormat, $userTZ, $gmtDates, $isGroupedReport, $auditedReport, $auditedAppliedFields, $auditedFieldType, $predefinedTemplates, $referenceAlias, $availableMassiveButtons);
	...
	static public function formatResultSet($reportId, $dashletId, $rs, & $resulsetFields, $userDateFormat, $userDateTimeFormat, $userTZ, $gmtDates, $isGroupedReport, $auditedReport, $auditedAppliedFields, $auditedFieldType, $predefinedTemplates, $referenceAlias, & $availableMassiveButtons) {
	...
	static private function formatResultSetRows($reportId, $dashletId, $resulsetFields, & $rowValues, $userDateFormat, $userDateTimeFormat, $userTZ, $gmtDates, $isGroupedReport, $auditedReport, $auditedAppliedFields, $auditedFieldType, $predefinedTemplates, $referenceAlias, & $availableMassiveButtons) {
			...
			if (!$isTotal) {
				...
				if (in_array($formatType, array("enum", "radioenum"))) {
					...
					$extraParams = array(
						...
						'currentRef' => $fieldDef['fieldReference'],
						'currentEnum' => $fieldDef['enumLabels'],
						'template' => $fieldDef['templates']['enum'],
						'enumTemplates' => $predefinedTemplates['enum'],
						'extraFormat' => $fieldDef['format']['extra'],
					);
					$returnedPremiumEnum = asol_CommonUtils::managePremiumFeature("commonEnumFormat", "commonFunctions.php", "getCommonEnumPresentationField", $extraParams);
					$rowValues[$fieldDef['alias']] = ($returnedPremiumEnum !== false ? $returnedPremiumEnum['value'] : (isset($fieldDef['enumLabels'][$currentUnformattedValue]) ? $fieldDef['enumLabels'][$currentUnformattedValue] : $currentUnformattedValue));
					$rowValues["asolRowStyle"] = ($returnedPremiumEnum["style"] !== null ? $returnedPremiumEnum["style"] : (isset($rowValues["asolRowStyle"]) ? $rowValues["asolRowStyle"] : null));
					$rowValues["asolRowClass"] = ($returnedPremiumEnum["class"] !== null ? $returnedPremiumEnum["class"] : (isset($rowValues["asolRowClass"]) ? $rowValues["asolRowClass"] : null));
					//***********************//
					//***AlineaSol Premium***//
					//***********************//
					...
					$extraParams = array(
						'currentValue' => $currentUnformattedValue,
						'currentRef' => $fieldDef['fieldReference'],
						'currentEnum' => $fieldDef['treeValues'],
						'extraFormat' => $fieldDef['format']['extra'],
					);
					$returnedPremiumTree = asol_CommonUtils::managePremiumFeature("commonTreeFormat", "commonFunctions.php", "getCommonTreePresentationField", $extraParams);
					$rowValues[$fieldDef['alias']] = ($returnedPremiumTree !== false ? $returnedPremiumTree['value'] : $currentUnformattedValue);
					$rowValues["asolRowStyle"] = ($returnedPremiumTree["style"] !== null ? $returnedPremiumTree["style"] : (isset($rowValues["asolRowStyle"]) ? $rowValues["asolRowStyle"] : null));
					$rowValues["asolRowClass"] = ($returnedPremiumTree["class"] !== null ? $returnedPremiumTree["class"] : (isset($rowValues["asolRowClass"]) ? $rowValues["asolRowClass"] : null));
					//***********************//
					//***AlineaSol Premium***//
					//***********************//
					...
					$calendarType = ( isset($fieldDef['format']['extra']['calendar']) ? $fieldDef['format']['extra']['calendar'] : $userConf['calendarType'] );
					...
					if (($currentUnformattedValue != "") && (date('Y-m-d', strtotime($currentUnformattedValue)) == $currentUnformattedValue)) {
						if ($calendarType == 'persian') {
							$rowValues[$fieldDef['alias']] = asol_CommonUtils::dateToPersian($currentUnformattedValue, false, $userDateFormat);
						} else {
					...
					if (($currentUnformattedValue != "") && (date('Y-m-d H:i:s', strtotime($currentUnformattedValue)) == $currentUnformattedValue)) {
						...
						if ($calendarType == 'persian') {
							$rowValues[$fieldDef['alias']] = asol_CommonUtils::dateToPersian($currentUnformattedValue, true, $userDateTimeFormat);
						} else {
					...
					$returnedPremiumNumeric = asol_CommonUtils::managePremiumFeature("commonNumericFormat", "commonFunctions.php", "getCommonNumericField", $extraParams);
					$rowValues[$fieldDef['alias']] = ($returnedPremiumNumeric !== false) ? $returnedPremiumNumeric["value"] : $currentUnformattedValue;
					$rowValues["asolRowStyle"] = ($returnedPremiumNumeric["style"] !== null ? $returnedPremiumNumeric["style"] : (isset($rowValues["asolRowStyle"]) ? $rowValues["asolRowStyle"] : null));
					$rowValues["asolRowClass"] = ($returnedPremiumNumeric["class"] !== null ? $returnedPremiumNumeric["class"] : (isset($rowValues["asolRowClass"]) ? $rowValues["asolRowClass"] : null));
					//***********************//
					//***AlineaSol Premium***//
					//***********************//
					...
					$returnedPremiumNumeric = asol_CommonUtils::managePremiumFeature("commonNumericFormat", "commonFunctions.php", "getCommonNumericField", $extraParams);
					$rowValues[$fieldDef['alias']] = ($returnedPremiumNumeric !== false) ? $returnedPremiumNumeric["value"] : asol_CommonUtils::currency_format($currentUnformattedValue);
					$rowValues["asolRowStyle"] = ($returnedPremiumNumeric["style"] !== null ? $returnedPremiumNumeric["style"] : (isset($rowValues["asolRowStyle"]) ? $rowValues["asolRowStyle"] : null));
					$rowValues["asolRowClass"] = ($returnedPremiumNumeric["class"] !== null ? $returnedPremiumNumeric["class"] : (isset($rowValues["asolRowClass"]) ? $rowValues["asolRowClass"] : null));
					//***********************//
					//***AlineaSol Premium***//
					...
					$returnedPremiumNumeric = asol_CommonUtils::managePremiumFeature("commonNumericFormat", "commonFunctions.php", "getCommonNumericField", $extraParams);
					$rowValues[$fieldDef['alias']] = ($returnedPremiumNumeric !== false) ? $returnedPremiumNumeric["value"] : $rowValues[$fieldDef['alias']];
					$rowValues["asolRowStyle"] = ($returnedPremiumNumeric["style"] !== null ? $returnedPremiumNumeric["style"] : (isset($rowValues["asolRowStyle"]) ? $rowValues["asolRowStyle"] : null));
					$rowValues["asolRowClass"] = ($returnedPremiumNumeric["class"] !== null ? $returnedPremiumNumeric["class"] : (isset($rowValues["asolRowClass"]) ? $rowValues["asolRowClass"] : null));
					//***********************//
					//***AlineaSol Premium***//
					...
					$returnedPremiumButton = asol_CommonUtils::managePremiumFeature("commonButtonFormat", "commonFunctions.php", "getCommonButtonField", $extraParams);
					$rowValues[$fieldDef['alias']] = ($returnedPremiumButton !== false ? $returnedPremiumButton['single'] : '');
					if (isset($fieldDef['format']['extra']['isMassive']) && ($fieldDef['format']['extra']['isMassive'])) {
						$availableMassiveButtons[$fieldDef['alias']] = ($returnedPremiumButton !== false ? $returnedPremiumButton['massive'] : '');
					}
					//***********************//
					//***AlineaSol Premium***//
					//***********************//
					...
					//***********************//
					$extraParams = array(
						'value' => $currentUnformattedValue,
						'extra' => $fieldDef['format']['extra'],
						'currentRef' => $fieldDef['fieldReference'],
						'currentRow' => $rowValues,
						'referenceAlias' => $referenceAlias,	
					);				
					$returnedPremiumImage = asol_CommonUtils::managePremiumFeature("commonMultimediaFormat", "commonFunctions.php", "getCommonMultimediaFormat", $extraParams);
					if ($returnedPremiumImage !== false) {
						$rowValues[$fieldDef['alias']] = $returnedPremiumImage[0];
					}
					//***********************//
					//***AlineaSol Premium***//
					//***********************//
				...
				if (in_array($fieldDef['grouping'], array("Minute Detail", "Quarter Hour Detail", "Hour Detail", "Day Detail", "DoW Detail", "WoY Detail", "Month Detail", "Natural Quarter Detail", "Fiscal Quarter Detail", "Natural Year Detail", "Fiscal Year Detail"))) {
					...
					$rowValues[$fieldDef['alias']] = self::formatDateSpecialsGroup($reportId, $dashletId, $rowValues[$fieldDef['alias']], $fieldDef, $userDateFormat, $userTZ, $gmtDates);
	...
	static public function applyGroupFormat($fieldValue, $fieldGroupingType, $userDateFormat) {
		...
		global $timedate, $mod_strings ,$current_language;
		...
		switch ($fieldGroupingType) {
			...
			case "Quarter Hour Grouped":
				...
				if($calendarType == 'persian' && $current_language == 'fa_ir' ) asol_CommonUtils::numbersToPersian($fieldValue);
			...
			case "Day Grouped":
				...
				if ((!$timedate->check_matching_format($fieldValue, $userDateFormat)) && ($fieldValue != "")){						
					if ($calendarType == 'persian') {
						$fieldValue = asol_CommonUtils::dateToPersian($fieldValue, false, $userDateFormat);
					} else {
			...
			case "WoY Grouped":
				...
				if ($calendarType == 'persian' && $current_language == 'fa_ir') {
					asol_CommonUtils::numbersToPersian($fieldValue);
				}
	...
	static public function validateSqlRemovedFields(& $sql, $alternativeDb, $reportTable, $hasDeleted) {
		...
		foreach ($fieldsToBeRemoved as $fieldToBeRemoved) {
			if (strpos($sql, $fieldToBeRemoved) !== false) {
				...
				asol_Reports::$reported_error = 'Forbidden field has been referenced at SQL query: '.$fieldToBeRemoved;
				break;
			}
		}
	...
	static private function replaceReportsFieldVars($textWithVars, $tableAliasName, $aliasIndexTable, $audited_report, $alternativeDb) {
		...
		$textWithVars = asol_CommonUtils::replaceStaticGlobalVars($textWithVars);
		$beanItems = asol_CommonUtils::getItemVariables($textWithVars);
		...
		foreach($beanItems as $beanItem) {
			...
			if (count($beanValues) == 2) {
				...
				if ($alternativeDb === false) {
					...
					$relatedTable = ($relatedTable == false ? BeanFactory::newBean(end($usedModules))->table_name : $relatedTable);
					$relatedTable = (empty($relatedTable)) ? strtolower(end($usedModules)) : $relatedTable;
				...
				foreach ($usedModules as $usedIndex=>$usedModule) {
					...
					if ($isNonCrmDatabase) {
						...
						if (isset($processedJoinsFromSqlKeys[$usedModule])) {
							...
							$mainPK = asol_Reports::getSelectionResults("SHOW KEYS FROM ".$usedModule." WHERE Key_name = 'PRIMARY'", null, true, $alternativeDb);
						...
						$relatedInfo = asol_Reports::getReportsRelatedFields($bean, $usedKeys[$usedIndex]);	
	...
	static private function propertyApplies($visibilityConfig){
		
		global $current_user;
		
		$propertyApplies = true;
		
		if(isset($visibilityConfig)){
		
			$extraParams = array(
					'properties' => json_decode(urldecode($visibilityConfig),true) ,
					'elementDomain' => $current_user->asol_domain_id 
			);
			
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
			$propertyApplies = !asol_CommonUtils::managePremiumFeature("propertiesManagement", "commonFunctions.php", "propertiesUnAppliesConditions", $extraParams );
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
	...
	static private function roleApplies($currentRoles){
		
		global $userRoles, $current_user;
		
		if(!isset($userRoles)){
						
			$userRoles = array();			
			
			foreach( ACLRole::getUserRoles($current_user->id, false) as $roleObject) {
				$userRoles[] = $roleObject->id;
			}
			
		}
		
		$roleApplies = array_intersect($userRoles, $currentRoles);
		$roleApplies = !empty($roleApplies);
						
		return ($roleApplies || empty($userRoles) || (empty($currentRoles)));
