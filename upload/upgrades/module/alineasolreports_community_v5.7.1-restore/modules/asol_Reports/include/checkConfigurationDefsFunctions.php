...
class asol_CheckConfigurationDefsFunctions {
	...
	static public $errorTips = array(
		...
		'asolReportsSystemCurlUsage' => 'The configured value must be a Boolean',
	...
	static public function checkConfiguration($configParam, $noLongerAvailable = false) {
		...
		if (isset($sugar_config[$configParam])) {
			...
			switch ($configParam) {
				...
				case "asolReportsSystemCurlUsage":
				...
				case "asolReportsTlConfigurations":
					if (!self::checkExternalTls()) {
						$hasError = true;
						$posibleResolution = "· Check your SugarCRM External TargetList Parameters.";
					}
					break;
							
				case "asolReportsFtpDirectories":
					if (!self::checkExternalFtps()) {
						$hasError = true;
						$posibleResolution = "· Check your SugarCRM External Ftps Parameters.";
					}
					break;
	...
	static private function getImgInfoIcon($configParam, $overrideConfigParam, $posibleValues, $httpRequestParam = false, $isError = true) {
		...
		if ($isError) {
			...
			return "<img id='".$configParam."_Info' style='display: ".($httpRequestParam ? 'none' : 'inline')."' border='0' class='inlineHelpTip' alt='Info' src='modules/asol_Reports/include/images/asol_reports_help_inline.gif' onClick='return SUGAR.util.showHelpTips(this, &quot;<div class=\\\"detail view  detail508\\\"><table class=\\\"list view\\\"><tr><td width=\\\"20%\\\"><b>Error:</b></td><td width=\\\"80%\\\">".$errorTip."</td></tr><tr><td width=\\\"20%\\\"><b>Posible Solutions:</b></td><td width=\\\"80%\\\">".$posibleValues."</td></tr></table></div>&quot;, &quot;&quot;, &quot;&quot;);'>";
		
		} else {
			
			return "<img id='".$configParam."_Info' style='display: ".($httpRequestParam ? 'none' : 'inline')."' border='0' class='inlineHelpTip' alt='Info' src='modules/asol_Reports/include/images/asol_reports_help_inline.gif' onClick='return SUGAR.util.showHelpTips(this, &quot;<div class=\\\"detail view  detail508\\\"><table class=\\\"list view\\\"><tr><td width=\\\"20%\\\"><b>Posible Values:</b></td><td width=\\\"80%\\\">".$posibleValues."</td></tr></table></div>&quot;, &quot;&quot;, &quot;&quot;);'>";
	...
	static private function checkExternalApps() {
		...
		if (is_array($sugar_config['asolReportsExternalApplications'])) {
			...
			foreach ($sugar_config['asolReportsExternalApplications'] as $extApp) {
				...
				if ((!isset($extApp["label"])) || (!isset($extApp["data"]["url"])) || (!isset($extApp["data"]["parameters"]["external"])) || (!isset($extApp["data"]["clean"]["headers"])) || (!isset($extApp["data"]["clean"]["quotes"]))) {
					
					$returnedVal = false; //Missing Values
					break;
					
				} else if ((!is_string($extApp["label"])) || (!is_string($extApp["data"]["url"])) || (!is_string($extApp["data"]["parameters"]["external"])) || (!is_bool($extApp["data"]["clean"]["headers"])) || (!is_bool($extApp["data"]["clean"]["quotes"]))) {
	...
	static private function checkExternalTls() {

		global $sugar_config;
	
		$returnedVal = true;
	
		if (is_array($sugar_config['asolReportsTlConfigurations'])) {
				
			foreach ($sugar_config['asolReportsTlConfigurations'] as $extTl) {
	
				if ((!isset($extTl["label"])) || (!isset($extTl["data"]["mode"])) || (!isset($extTl["data"]["id"])) || (!isset($extTl["data"]["name"]))) {
						
					$returnedVal = false; //Missing Values
					break;
						
				} else if ((!is_string($extTl["label"])) || (!is_string($extTl["data"]["mode"])) || (!is_string($extTl["data"]["id"])) || (!is_string($extTl["data"]["name"]))) {
	...
	static private function checkExternalFtps() {
	
		global $sugar_config;
	
		$returnedVal = true;
	
		if (is_array($sugar_config['asolReportsFtpDirectories'])) {
				
			foreach ($sugar_config['asolReportsFtpDirectories'] as $extFtp) {
	
				if ((!isset($extFtp["label"])) || (!isset($extFtp["data"]["host"])) || (!isset($extFtp["data"]["secure"])) || (!isset($extFtp["data"]["username"])) || (!isset($extFtp["data"]["password"]))) {
						
					$returnedVal = false; //Missing Values
					break;
						
				} else if ((!is_string($extFtp["label"])) || (!is_string($extFtp["data"]["host"])) || (!is_bool($extFtp["data"]["secure"])) || (!is_string($extFtp["data"]["username"])) || (!is_string($extFtp["data"]["password"]))) {
