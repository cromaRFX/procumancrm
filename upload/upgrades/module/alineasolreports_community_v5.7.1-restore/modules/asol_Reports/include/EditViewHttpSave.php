...
<html>
	...
	<head>
		...
		<script type="text/javascript">
			...
			'.asol_ReportsManagementFunctions::getInitReportsJavaScriptsHtml($hasPremiumFeatures, $isMetaReport, $externalApps, $reportType[0], $reportScheduledType[0]).'
						...
						<form id="create_form" name="create_form" method="post" action="index.php">
							...
							<div id="DEFAULT" class="alineasol_reports yui-navset detailview_tabs yui-navset-top">';
								...
								if($manageReportTabs === false){
									...
									echo $manageReportTabs['html'].$manageReportTabs['css']; 
							...
							echo '
									...
									<input type="hidden" id="asolReportsAvailableRoles" value="'.rawurlencode(json_encode(asol_CommonUtils::getCurrentRoles())).'">';
									//***********************//
									//***AlineaSol Premium***//
									//***********************//
												...
												<tr>
													...
													<td width="100%" valign="top">
														<div id="reportEditorDiv">
															<span style="font-weight:bold;">'.$mod_strings['LBL_REPORT_TITLE'].':</span><input type="text" id="asolReportTitle" style="margin:10px 5px" value="'.$reportFieldsArray['tables'][0]['title']['text'].'">';
															...
															echo $returnedPremiumMultiLanguageHtml;
															//***********************//
															//***AlineaSol Premium***//
															//***********************//
															
													echo	'<div id="reportFieldsDiv" class="mainReport">
																'.asol_ReportsManagementFunctions::getFieldsHeadersHtml($focus->alternative_database, $focus->row_index_display, $reportTemplates, $reportFieldsArray['tables'][0]['config'], $reportFieldsArray['tables'][0]['joins'], $reportCss).'
															</div>
									...
									if (asol_CommonUtils::isDomainsInstalled()) {
										...
										<div id="domainPublishing" class="reportPanel">
											...
											<table class="edit view" style="'.$domainPublishVisibility.'">
												<tbody>
													<tr>
														'.asol_manageDomains::getBeanDomainNameHtml($focus->asol_domain_id, $focus->asol_domain_name).'
