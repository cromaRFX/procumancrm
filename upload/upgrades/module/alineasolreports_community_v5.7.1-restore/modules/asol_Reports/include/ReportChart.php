...
class asol_ReportsCharts {
	...
	static public function getDataForChartsGeneration(& $chartInfo, & $chartConfig, $fieldValues, $subTotalsCharts, $subGroupsExport, $massiveData, $isDetailReport, $isGroupedReport, $hasFunctionField, $groups, $groupExport, $userDateFormat) {
		...
		foreach ($chartInfo as $chartKey=>$currentChartInfo) {
			...
			$currentChartLabel = trim(is_array($currentChartInfo['label']) ? $currentChartInfo['label']['text'] : $currentChartInfo['label']);
				...
				if ($isDetailReport) {
					...
					if (in_array($currentChartInfo['type'], array('pie', 'donut', 'bar', 'funnel', 'stack', 'horizontal', 'line', 'scatter', 'area', 'bubble'))) {
						...
						foreach ($subTotalsCharts as $key=>$values) {
							$subGroupsChart[$currentChartLabel][$key] = asol_ReportsGenerateQuery::getGroupingChartValue($groups, $values, $userDateFormat);
	...
	static public function getChartFilesWithExtraData($chartsEngine, $isStackedChart, $massiveData, & $chartInfo, $chartConfigs, $chartTemplates, $predefinedColorTemplates, $chartYAxisLabels, $chartValues, $subGroupsChart, $reportId, $dashletId, $reportModule, $chartsHttpQueryUrls, $isDetailedReport, $isGroupedReport, $isStoredReport) {
		...
		foreach ($chartValues as $currentChartValues) {
			...
			$currentChartLabel = (is_array($currentChartInfo['label']) ? $currentChartInfo['label']['text'] : $currentChartInfo['label']);
			$currentSubGroupsChart = $subGroupsChart[$currentChartLabel];
				
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
			if (is_array($currentChartInfo['label'])) {
				$extraParams = array('label' => $currentChartInfo['label']);
				...
				$currentChartInfo['label'] = ($returnedMultiLanguage !== false ? $returnedMultiLanguage : $currentChartInfo['label']['text']);
			}
			//***********************//
			//***AlineaSol Premium***//
			//***********************//
			...
			if (in_array($currentChartInfo['type'], array('pie', 'donut'))) {
				if ($chartsEngine != 'nvd3')
					$urlChart[] = self::generateCrmNativeNormalChartData($currentChartTypes[0], count($urlChart), $reportModule, $currentChartInfo['label'], $currentSubGroupsChart, $currentChartValues['mainChart'], count($currentSubGroupsChart), (empty($chartsHttpQueryUrls)) ? null : $chartsHttpQueryUrls[count($urlChart)], $currentChartInfo['function'], $isStoredReport);
				else
					$urlChart[] = self::generateNvd3NormalChartData($reportId, $dashletId, $currentChartTypes, $chartYAxisLabels[$chartIndex], $currentChartYAxisSides, $currentChartConfigs, $currentChartTemplates, $predefinedColorTemplates, count($urlChart), $reportModule, $currentChartInfo['label'], $currentSubGroupsChart, $currentChartValues, count($currentSubGroupsChart), (empty($chartsHttpQueryUrls)) ? null : $chartsHttpQueryUrls[count($urlChart)], $currentChartInfo['function'], $isStoredReport, $currentChartInfo['halfChart']);
				$chartInfo[$chartIndex]['type'] = 'PieChart';
				$chartInfo[$chartIndex]['subgroups'] = count($currentSubGroupsChart);
				$chartSubGroupsValues[] = count($currentSubGroupsChart);
			...
			if ($currentChartInfo['type'] == 'stack') {
				...
				if ($chartsEngine != 'nvd3') {
					if ($isStackedChart)
						$urlChart[] = self::generateCrmNativeStackedChartData($currentChartTypes[0], count($urlChart), $reportModule, $currentChartInfo['label'], $currentSubGroupsChart, $currentChartValues['mainChart'], count($currentSubGroupsChart), (empty($chartsHttpQueryUrls)) ? null : $chartsHttpQueryUrls[count($urlChart)], $currentChartInfo['function'], $isStoredReport);
					else
						$urlChart[] = self::generateCrmNativeNormalChartData($currentChartTypes[0], count($urlChart), $reportModule, $currentChartInfo['label'], $currentSubGroupsChart, $currentChartValues['mainChart'], count($currentSubGroupsChart), (empty($chartsHttpQueryUrls)) ? null : $chartsHttpQueryUrls[count($urlChart)], $currentChartInfo['function'], $isStoredReport);
				} else {
					if ($isStackedChart)
	...
	static public function getChartEngineLibraries($chartEngine, $isDashlet, $isPreview, $callback = null) {
		...
		$reportsLibrariesPath = 'modules/asol_Reports/include/js';
		...
		foreach ($chartEngineLibraries as $chartEngineLibrary) {
			...
			if ($library[0] == 'JS') {
				if ($library[1] === '0') {
					...
					$chartFilesHtml .= '<script type="text/javascript"> if (typeof '.$library[1].' === "undefined") { $LAB.script("'.$library[2].'")'.(isset($library[3]) ? '.wait().script("'.$library[3].'")' : '').(isset($library[4]) ? '.wait().script("'.$library[4].'")' : '').'.wait('.($callback !== null ? $callback : '').'); } else { $LAB.script("'.$library[3].'")'.(isset($library[4]) ? '.wait().script("'.$library[4].'")' : '').'.wait('.($callback !== null ? $callback : '').'); } </script>';
				}
	...
	static private function getNvd3DataLabelsJs($indexKey, $chartLegends) {
		...
		$jsData = 'var dataLabels_'.$indexKey.' = ['.(!empty($chartLegends) ? '"'.implode('","', $chartLegends).'"' : '').'];';
